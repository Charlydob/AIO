...
const Fin = {
  ...
  async saveEntry(){
    ...
    let rtdbOK=true;
    try{
      if(EDIT_ID) await db.ref(`${base}/${eid}`).update(entry);
      else await db.ref(`${base}/${eid}`).set({...entry,id:eid});
    }catch(e){ rtdbOK=false; console.error("RTDB fin set:", e); }

    // Mirror LS
    const store = lsGet(LS_KEY_FIN(), {entries:{}});
    store.entries[eid] = {...entry, id:eid};
    lsSet(LS_KEY_FIN(), store);
  ...
  async deleteEntry(){
    if(!EDIT_ID) return;
    let rtdbOK=true;
    try{ await db.ref(`finance/${UID}/entries/${EDIT_ID}`).remove(); }
    catch(e){ rtdbOK=false; console.error("RTDB fin del:", e); }
    const store = lsGet(LS_KEY_FIN(), {entries:{}});
...
async function loadEntries(){
  ...
  }catch(e){ console.warn("RTDB fin read:", e); }
  if(!obj){ obj = lsGet(LS_KEY_FIN(), {entries:{}}).entries; }
